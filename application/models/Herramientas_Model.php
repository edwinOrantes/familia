...
class Herramientas_Model extends CI_Model {
    ...
    public function facturacionDiaria($fecha){
        ...
        $sql = "SELECT SUM(fe.totalFactura) AS facturado FROM tbl_facturas_emitidas AS fe
    ...
    public function facturadoMes($i, $f){
        $sql = "SELECT SUM(totalFactura) AS facturado FROM tbl_facturas_emitidas WHERE DATE(fechaFactura) BETWEEN  '$i' AND '$f' ";
        $datos = $this->db->query($sql);
        return $datos->row();
    }
