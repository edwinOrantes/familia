...
class Botiquin_Model extends CI_Model {
    ...
    public function actualizarMedicamentoFactura($data = null){ 
        if($data != null){
            $sql = "UPDATE tbl_factura_medicamentos SET cantidad = ? WHERE idFacturaMedicamento = ? ";
            if($this->db->query($sql, $data)){
                return true;
            }else{
                return false;
    ...
    // Detalle de stocks
        ...
        public function detalleStock(){
            $sql ="SELECT ds.idDetalleStock, ds.idInsumo, m.codigoMedicamento, m.nombreMedicamento, ds.stockInsumo, ds.debeInsumo, ds.idStock
                   FROM tbl_detalle_stock AS ds
                   INNER JOIN tbl_medicamentos AS m ON(ds.idInsumo = m.idMedicamento)";
            $datos = $this->db->query($sql);
            return $datos->result();
