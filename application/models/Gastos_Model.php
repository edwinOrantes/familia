...
class Gastos_Model extends CI_Model {
    ...
    public function obtenerGastos($i, $f){
        /* $sql = "SELECT cg.idCuenta, cg.nombreCuenta, tg.nombreTipoGasto, g.* FROM tbl_gastos as g INNER JOIN tbl_tipo_gasto as tg ON(g.tipoGasto = tg.idTipoGasto)
                INNER JOIN tbl_cuentas_gastos as cg ON(g.idCuentaGasto = cg.idCuenta) WHERE g.fechaGasto BETWEEN '$i' AND '$f' ORDER BY g.idCuentaGasto DESC"; */
        /* $sql = "SELECT ccg.idClasificacionCG, cg.idCuenta, cg.nombreCuenta, g.* 
    ...
    public function actualizarGastos($data = null){
        if($data != null){
            if(isset($data["numeroGasto"])){
                $sql = "UPDATE tbl_gastos SET tipoGasto = ?, montoGasto = ?, entregadoGasto = ?, idCuentaGasto = ?, fechaGasto = ?,
