...
class Laboratorio_Model extends CI_Model {
    ...
    /* Metodos para gestion de procesos de laboratorio  */
            ...
            if ($data["hoja"] != null) {
                ...
                if($paciente["idPaciente"] == 0){
                    ...
                    if($this->db->query($sql, $paciente)){
                        ...
                        $hojaCobro["anulada"]  = $hoja["anulada"];
                ...
                $sqlL = "INSERT INTO tbl_consulta_laboratorio(idHoja, idPaciente, codigoConsulta) VALUES('$idHoja', '$pacienteL', $cod)";
                $this->db->query($sqlL);
        ...
        public function detalleConsulta($id){
            /* $sql = "SELECT cl.idConsultaLaboratorio, p.nombrePaciente, p.edadPaciente,m.nombreMedico, cl.codigoConsulta, cl.fechaConsultaLaboratorio FROM
                    tbl_consulta_laboratorio as cl INNER JOIN tbl_hoja_cobro as hc on(cl.idHoja = hc.idHoja) INNER JOIN tbl_pacientes
                    AS p ON(hc.idPaciente = p.idPaciente) INNER JOIN tbl_medicos AS m ON(hc.idMedico = m.idMedico) 
                    WHERE idConsultaLaboratorio = '$id' "; */
            $sql = "SELECT cl.idConsultaLaboratorio, cl.online, p.nombrePaciente, p.edadPaciente, cl.codigoConsulta, cl.fechaConsultaLaboratorio,
                    m.nombreMedico , tcl.nombreTipoConsultaLab
