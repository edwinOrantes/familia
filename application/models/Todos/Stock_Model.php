...
class Stock_Model extends CI_Model {
    ...
    public function guardarDetalle($data = null){
        if($data != null){
            $sql = "INSERT INTO tbl_detalle_stock(idStock, idInsumo, stockInsumo, debeInsumo) VALUES(?, ?, ?, ?)";
            if($this->db->query($sql, $data)){
                return true;
            }else{
                return false;
    ...
    public function actualizarDetalleStock($data = null){
        if($data != null){
            $sql = "UPDATE tbl_detalle_stock SET stockInsumo = ?, debeInsumo = ? WHERE idDetalleStock = ?";
            if($this->db->query($sql, $data)){
                return true;
            }else{
                return false;
