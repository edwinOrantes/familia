...
class Reportes extends CI_Controller {
    ...
    public function externos_por_hoja(){
		...
		$sheet->setCellValue('A1', 'Hoja de cobro');
		// $sheet->setCellValue('B1', 'Recibo de cobro');
	...
	public function liquidacion_caja(){
		...
		//
			$correlativo = $this->Reportes_Model->numeroCorrelativo(); // Ultimo cuenta cobrada
			$ultimoLiquidado = $this->Reportes_Model->ultimoLiquidacionGenerada();
			$data["numeroLimite"] = $correlativo->correlativo;
		...
		for ($i=$inicio; $i <= $fin; $i++) { 
			...
			if(sizeof($honorarios) > 0){ // Porque hay hojas de cobro que no tienen externos
				foreach ($honorarios as $honorario) {
					...
					$arregloHonorario["precio"] = $honorario->precioExterno;
